...
// UPDATED: Removed "Education" from this list
const navLinks = [
  { href: "#home", text: "Home" },
  ...
  return (
    ...
    >
      <a
        ...
        onClick={handleClick}
        className={cn(
          "relative block rounded-full px-4 py-2 text-sm font-medium transition-all duration-300 hover:bg-white/10 hover:text-white",
          props.i === 0 ? "text-white" : "text-muted-foreground"
        )}
      >
        {props.text}
      </a>
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:site_name" content="PreetamV" />
        <meta property="og:description" content={meta.description} />
        <meta property="og:title" content={meta.title} />
        <meta property="og:image" content={meta.image} />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:site" content="PreetamV" />
      ...
      <nav
        className={cn(
          ...
          isScrolled
            ? "bg-gradient-to-br from-background/80 to-transparent shadow-sm backdrop-blur-md transition"
            : "bg-transparent"
        )}
      >
        ...
        <Link href="/">
          <span className="text-lg font-semibold tracking-tight">Preetam.</span>
        </Link>

        {/* Centered Desktop Menu */}
        <ul className="hidden sm:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center gap-1 rounded-full border border-white/10 bg-black/20 p-1.5 px-2 backdrop-blur-md transition-all hover:border-white/20 hover:bg-black/30 shadow-lg">
          {navLinks.map((link, i) => (
            <NavItem
              key={link.href}
              href={link.href}
              ...
              <div className="flex h-full flex-col items-start justify-between overflow-y-auto">
                ...
                <div className="flex min-h-fit w-full flex-col space-y-8 px-[22px] py-10">
                  <span className="text-sm text-muted-foreground">
                    © {new Date().getFullYear()} Preetam. All rights reserved.
                  </span>
                </div>
              </div>
            </motion.div>
